...
import org.example.server.http.HttpServerInitializer;
...
@ConditionalOnProperty(prefix = "socket", name = "enable", havingValue = "true", matchIfMissing = false)
    ...
    public void start() {
        ...
        try {
            ...
            serverBootstrap.group(bossGroup, workGroup) //设置线程组
                    ...
                    .childHandler(new SocketServerInitializer())// 给workGroup的EventLoop对应的管道设置处理器
            ;
        ...
        log.info("socket server started in " + serverPort);
    }
