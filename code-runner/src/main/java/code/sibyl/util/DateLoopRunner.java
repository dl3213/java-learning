...
public class DateLoopRunner {
    ...
    public void run(Consumer<String> consumer) {
        Date date = DateUtil.parseDate(from, this.parsePattern);
        Date now = StringUtils.isNotBlank(to) ? DateUtil.parseDate(to, this.parsePattern) : new Date();
        if (this.parallel) {
    ...
    public static void main(String[] args) {
        DateLoopRunner
                ...
                .function(d -> DateUtil.addMonths(d, 1))
